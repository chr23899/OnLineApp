...
namespace Chr.OnlineApp.DAL.SqlServer
{
    ...
    /// </summary>
    public class YWAssignmentDAL : DAL.Common.YWAssignmentDAL
    {
        ...
        public override int Insert(YWAssignment yWAssignment)
        {
            string sqlText = "INSERT INTO [YW_Assignment]"
            ...
            {
                new SqlParameter("@planId"         , SqlDbType.Int      , 4  ){ Value = yWAssignment.PlanId         },
                new SqlParameter("@planName"       , SqlDbType.NVarChar , 50 ){ Value = yWAssignment.PlanName       },
                new SqlParameter("@courseId"       , SqlDbType.Int      , 4  ){ Value = yWAssignment.CourseId       },
                new SqlParameter("@courseName"     , SqlDbType.NVarChar , 50 ){ Value = yWAssignment.CourseName     },
                new SqlParameter("@createUserName" , SqlDbType.NVarChar , 20 ){ Value = yWAssignment.CreateUserName },
                new SqlParameter("@createUserId"   , SqlDbType.Int      , 4  ){ Value = yWAssignment.CreateUserId   },
                new SqlParameter("@updateUserName" , SqlDbType.NVarChar , 20 ){ Value = yWAssignment.UpdateUserName },
                new SqlParameter("@updateUserId"   , SqlDbType.Int      , 4  ){ Value = yWAssignment.UpdateUserId   },
                new SqlParameter("@title"          , SqlDbType.NVarChar , 50 ){ Value = yWAssignment.Title          },
                new SqlParameter("@content"        , SqlDbType.NVarChar , 500){ Value = yWAssignment.Content        },
                new SqlParameter("@link"           , SqlDbType.NVarChar , 500){ Value = yWAssignment.Link           },
        ...
        /// </summary>
        /// <param name="yWAssignment">基础对象名称（YWAssignment）实例对象</param>
        public override int Update(YWAssignment yWAssignment)
        {
            string sqlText = "UPDATE [YW_Assignment] SET "
        ...
        {
            string sqlText = "DELETE FROM [YW_Assignment] "
                           + "WHERE [Id]=@Id";
            SqlParameter[] parameters = 
            {
        ...
        /// </summary>
        /// <param name="id">基础对象名称（YWAssignment）的主键“（Id）”</param>
        public override YWAssignment GetDataById(int id)
        {
            YWAssignment yWAssignment = null;
            ...
            {
                yWAssignment = new YWAssignment();
                ReadYWAssignmentAllData(sqlDataReader, yWAssignment);
            }
            sqlDataReader.Close();
            return yWAssignment;
        }
        ...
        /// </summary>
        public override List<YWAssignment> GetAllList()
        {
            string sqlText = "SELECT [Id],[planId],[planName],[courseId],[courseName],[createUserName],[createUserId],[updateUserName],[updateUserId],[title],[content],[link],"
                           + "[createTime],[updateTime],[status],[Alternate1],[Alternate2],[Alternate3],[Alternate4],[Alternate5] "
                           + "FROM [YW_Assignment]";
            List<YWAssignment> list = new List<YWAssignment>();
            SqlDataReader sqlDataReader = SFL.SqlHelper.ExecuteReader(sqlText, null);
            while (sqlDataReader.Read())
            {
                YWAssignment yWAssignment = new YWAssignment();
                ReadYWAssignmentAllData(sqlDataReader, yWAssignment);
                list.Add(yWAssignment);
            }
            sqlDataReader.Close();
            return list;
        }
